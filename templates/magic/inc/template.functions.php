...
function templateDisplayCSPanel() {
	...
	if($siegeData['castle_data'][_CLMN_MCD_OCCUPY_] == 1) {
		$guild = guildProfile($siegeData['castle_data'][_CLMN_MCD_GUILD_OWNER_]);
		$guildOwnerMark = $siegeData['castle_owner_alliance'][0][_CLMN_GUILD_LOGO_];
		$master = playerProfile($siegeData['castle_owner_alliance'][0][_CLMN_GUILD_MASTER_]);
	} else {
		$guild = '-';
	...
	$logo = returnGuildLogo($guildOwnerMark, 100);
	
	
	echo '<div class="hero-guide-block">';
		echo '<div class="cs-block">';
			echo $logo;
			echo '<span class="cs-guild-title">';
				echo $guild;
			echo '</span>';
			echo '<span class="cs-guild-guildowner">'.lang('csbanner_txt_1').'</span>';
		echo '</div>';
		echo '<div class="time-cs">';
			echo '<span class="Battle-Begins-In">'.lang('csbanner_txt_2').'</span>';
			echo '<br />';
